...
redbud_data_file <- "npn-data/redbud-open-flowers.csv"
...
# Occasionally there are two observations for a plant-phenophase in one day with 
# different intensity or status values. Keeping observation that was in phase 
# with highest intensity value (or observation that doesn't have intensity = NA)
df <- df %>%
...
# Address a few data entry issues (all of which will be addressed in new NN app)
  ...
  # Remove observations with status_flowers = NA & status_open = 0
  flowers <- flowers %>%
...
# First, create a function that returns the max value of a vector or an NA if
# all values are NA.
max_na <- function(x) {
  ifelse(sum(is.na(x)) == length(x), NA, max(x, na.rm = TRUE))
}
# Combine observations from the same day
flowers <- flowers %>%
...
# Remove observations past DOY 180 (redbuds flower early in the year)
flowers <- filter(flowers, doy <= 180)

# Visualize number of flowers/open flowers by week each year ------------------#
...
flowerssub <- flowers %>%
  ...
  filter(wk %in% 9:25)
# Number of trees
length(unique(flowerssub$id)) # 455 trees
...
# Exclude plant-years if:
  # <2 non-zero counts
  # No observations before DOY 100
  # Start and end observations are not 0 (indicating observations didn't span
...
# Here, using date(s) of highest open flower count. Given this, we'll want to 
# filter out plant-years where there were long gaps either before or after the 
# date when the highest number of open flowers were reported (to limit bias)
...
# there were more than 30 days before or after dates when max open flower
# counts reported
of_plantyr <- of %>%
...
# Calculate peak date as mean date between first and last dates with maximum
# open flower count
ofpeak <- of_plantyr %>%
  filter(filter30 == 0) %>%
  select(-filter30) %>%
...
# Get site locations that were used in models of peak open flower abundance
ofmax_sites <- ofmax %>%
